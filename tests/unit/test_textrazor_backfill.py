...
from seo_rank.cli import RAW_RESPONSE_SCHEMA, build_raw_response_record
...
def test_run_stored_run_live_textrazor_only_skips_existing_entity_urls(
    tmp_path: Path,
    monkeypatch,
) -> None:
    run_dir = tmp_path / "artifacts"
    ...
    existing_entity = build_raw_response_record(
        ...
        response=(
            ...
            | {"response": {**response["response"], "entities": response["response"]["entities"][:1]}}
        )(
